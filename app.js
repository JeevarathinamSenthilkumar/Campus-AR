...
// Coordinates of Engineering Building
...
function trackArrival(destLat, destLon) {
  ...
  navigator.geolocation.watchPosition(pos => {
    ...
    if (d < 10) alert(`🎯 You’ve reached ${TARGET_BUILDING.name}!`);
  });
}
...
window.onload = async () => {
  ...
  const coords = await getRoute(start, [TARGET_BUILDING.longitude, TARGET_BUILDING.latitude]);
  placeArrows(coords);
  trackArrival(TARGET_BUILDING.latitude, TARGET_BUILDING.longitude);
};
